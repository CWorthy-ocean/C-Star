...
class SlurmJob:
    ...
    def __init__(
        ...
        default_name = f"slurm_job_{datetime.strftime(datetime.now(),format='%Y%m%d_%H%M%S')}"

        self.script_path = Path.cwd() / f"{default_name}.sh" if script_path is None else Path(script_path)
        ...
        self.output_file = Path.cwd() / f"{default_name}.out" if output_file is None else output_file
        self.queue_name = scheduler.primary_queue_name if queue_name is None else queue_name
        self.queue = scheduler.get_queue(queue_name)
        
        if walltime > self.queue.max_walltime:
            raise ValueError(f"Selected walltime {walltime} exceeds maximum "+
                             f"walltime for selected queue {queue_name}: "+
                             f"{queue_instance.max_walltime}")
        else:
            self.walltime = walltime
            
        if cpus > (scheduler.max_cpus_per_node * queue_instance.max_nodes):
            raise ValueError(f"Selected number of CPUs: {cpus} unsupported "+
                             f"for selected queue {queue_name} with max nodes "+
                             f"{queue_instance.max_nodes} on system with global "+
                             f"max CPUs per node {scheduler.max_cpus_per_node}")
        else:
            self.cpus = cpus
    ...
    def submit(self):
        ...
        result = subprocess.run(
            f"sbatch {self.script_path}", shell=True, cwd={self.run_path}, env=slurm_env
        )

        if result.exitcode != 0:
            raise RuntimeError("Non-zero exit code when submitting job. STDERR: "
                               +f"\n{result.stderr}")
