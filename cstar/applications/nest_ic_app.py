...
class NestIcRunner(XBlueprintRunner[NestIcBlueprint]):
    ...
    def _create_initial_conditions(
        ...
        rst = RestartFile(path=self.blueprint.restart_file)
        restart_date = rst.timestamp

        parent_grid = Grid(filename=self.blueprint.parent_grid_path)

        # build InitialConditions kwargs
        ic_kwargs = dict(
            grid=Grid(filename=self.blueprint.child_grid_path),
        ...
        if self._has_bgc(self.blueprint.restart_file):
