...
Forward-migration of on-disk ``forge_blueprint.yaml`` data across
``forge_blueprint_version`` bumps lives in ``cstar.applications.forge.migration``, not
here — kept separate so that importing this module (the schema itself) for
validation-only uses stays light; see ``ForgeBlueprint._migrate_and_clean``, which
imports it lazily.
...
class ForgeBlueprint(Blueprint):
    ...
    def _migrate_and_clean(cls, data: Any) -> Any:
        ...
        ``ForgeBlueprint.from_yaml`` at all. See
        ``cstar.applications.forge.migration.migrate_forge_blueprint_data``.

        Imported here (inside the method, not at module level) so that a plain
        ``import cstar.applications.forge.blueprint`` never pulls in
        ``migration.py`` -- keeps this module's own import graph light for
        validation-only callers (see
        ``test_forge_blueprint_import_stays_light``).
        """
    ...
    def from_yaml(cls, path: str | Path) -> ForgeBlueprint:
        ...
        ``cstar.applications.forge.migration.migrate_forge_blueprint_data`` here.
        """
