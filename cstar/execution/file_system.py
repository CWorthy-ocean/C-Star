...
class JobFileSystemManager(LoggingMixin):
    ...
    def input_dir(self) -> Path:
        ...
        """
        input_root = DirectoryManager.data_home() / self.root
        return input_root / self._INPUT_NAME
    ...
    def work_dir(self) -> Path:
        ...
        system.
        """
        work_root = DirectoryManager.state_home() / self.root
        return work_root / self._WORK_NAME
    ...
    def tasks_dir(self) -> Path:
        ...
        """
        task_root = DirectoryManager.state_home() / self.root
        return task_root / self._TASKS_NAME

    @property
    def logs_dir(self) -> Path:
        """The directory for log files created during the job."""
        log_root = DirectoryManager.state_home() / self.root
        return log_root / self._LOGS_NAME

    @property
    def output_dir(self) -> Path:
        """The directory for writing any outputs from the job."""
        output_root = DirectoryManager.state_home() / self.root
        return output_root / self._OUTPUT_NAME
...
class RomsFileSystemManager(JobFileSystemManager):
    ...
    def compile_time_code_dir(self) -> Path:
        ...
        code_root = DirectoryManager.state_home() / self.root
        return code_root / self._COMPILE_TIME_NAME

    @property
    def runtime_code_dir(self) -> Path:
        """The directory for runtime code."""
        code_root = DirectoryManager.state_home() / self.root
        return code_root / self._RUNTIME_NAME

    @property
    def input_datasets_dir(self) -> Path:
        """The directory for input datasets."""
        data_root = DirectoryManager.data_home() / self.root
        return data_root / self._INPUT_DATASETS_NAME

    @property
    def _codebases_dir(self) -> Path:
        """The directory for codebases."""
        code_root = DirectoryManager.state_home() / self.root
        return code_root / self._CODEBASES_NAME

    @property
    def joined_output_dir(self) -> Path:
        """The directory for de-partitioned outputs."""
        data_root = DirectoryManager.data_home() / self.root
        return data_root / self._JOINED_OUTPUT_NAME
