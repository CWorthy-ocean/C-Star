...
def test_timeconstrainedrunrequestenricher_default_localcomputespec() -> None:
    ...
    assert f"timeout {compute.max_walltime}" in " ".join(enriched_request.command)
    # confirm the force-kill period is specified
    exp_fk = f"{TimeConstrainedRunRequestEnricher.ARG_FORCEKILL_TIMEOUT} {compute.force_kill_timeout}"
...
@pytest.mark.parametrize(
    ...
    [
        (
            LocalComputeSpec(max_walltime="100s", force_kill_timeout="9s"),
            "timeout 100s -k 9s",
        ),
        (
            LocalComputeSpec(max_walltime="10s", force_kill_timeout="10s"),
            "timeout 10s -k 10s",
        ),
        (
            LocalComputeSpec(max_walltime="10s"),
            "timeout 10s -k 2s",  # expect default fk timeout
        ),
        (
            LocalComputeSpec(force_kill_timeout="99s"),
