...
class TestRegistry:
    def test_register_and_get_retriever(self, mock_sourcedata_local_text_file):
        """Tests that a new retriever can be registered and fetched from the register"""
        retriever.register_retriever(DummyRetriever)
        r = retriever.get_retriever(mock_sourcedata_local_text_file())
        assert isinstance(r, DummyRetriever)

    def test_get_retriever_not_registered(self, mock_sourcedata_factory):
        """Tests that getting an unregistered retriever raises a ValueError"""
        sourcedata = mock_sourcedata_factory(
...
class TestRetrieverABC:
    def test_save_creates_dir_and_calls_subclass_save(
        self, tmp_path, mock_sourcedata_local_text_file
    ):
        """Tests that parent class save ensures validity before calling subclass save()"""
        r = DummyRetriever(mock_sourcedata_local_text_file)
    ...
    def test_save_raises_if_not_directory(
        ...
        r = retriever.LocalFileRetriever(mock_sourcedata_local_text_file)
        with pytest.raises(ValueError):
            r.save(file_path)


class TestRemoteFileRetriever:
    def test_read_returns_content(self, mock_sourcedata_remote_text_file):
        ...
        with mock.patch("cstar.io.retriever.requests.get", return_value=fake_response):
            r = retriever.RemoteTextFileRetriever(mock_sourcedata_remote_text_file())
            result = r.read()
        assert result == b"abc"
...
class TestRemoteTextFileRetriever:
    def test_save_writes_file(self, tmp_path, mock_sourcedata_remote_text_file):
        ...
        with mock.patch.object(
            ...
            r = retriever.RemoteTextFileRetriever(
                source=mock_sourcedata_remote_text_file()
            )
...
class TestRemoteBinaryFileRetriever:
    def test_save_writes_file(self, tmp_path, mock_sourcedata_remote_file):
    ...
    def test_save_raises_on_hash_mismatch(self, tmp_path, mock_sourcedata_remote_file):
...
class TestLocalFileRetriever:
    def test_read_and_save(self, tmp_path, mock_sourcedata_local_text_file):
        """Tests that LocalFileRetriever.read and .save read and copy files, respectively"""
        test_file = tmp_path / "f.txt"
        source = mock_sourcedata_local_text_file(location=test_file)
...
class TestRemoteRepositoryRetriever:
    def test_read_raises(self, mock_sourcedata_remote_repo):
    ...
    def test_save_clones_and_checkouts(self, tmp_path, mock_sourcedata_remote_repo):
    ...
    def test_save_raises_if_dir_not_empty(self, tmp_path, mock_sourcedata_remote_repo):
