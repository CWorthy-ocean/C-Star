...
class TestRemoteFileRetriever:
    def test_read_returns_content(self):
        ...
        with mock.patch("cstar.io.retriever.requests.get", return_value=fake_response):
            ...
            src = mock.Mock(location="http://example.com/file.txt")
            result = r.read(src)
        assert result == b"abc"
...
class TestRemoteTextFileRetriever:
    def test_save_writes_file(self, tmp_path):
        ...
        with mock.patch.object(
            ...
            r = retriever.RemoteTextFileRetriever()
            result = r._save(tmp_path, fake_source)

        # Ensure read was called with our source
        mock_read.assert_called_once_with(source=fake_source)
...
class TestRemoteBinaryFileRetriever:
    def test_save_writes_file_and_checks_hash(self, tmp_path):
        ...
        with mock.patch("cstar.io.retriever.requests.get", return_value=fake_response):
            ...
            path = r._save(tmp_path, fake_source)
...
class TestRemoteRepositoryRetriever:
    ...
    def test_save_clones_and_checkouts(self, tmp_path):
        ...
        with (
            ...
            r = retriever.RemoteRepositoryRetriever()
            result = r._save(tmp_path, src)

        mock_clone.assert_called_once_with(source_repo="repo-url", local_path=tmp_path)
        mock_checkout.assert_called_once_with(
            source_repo="repo-url", local_path=tmp_path, checkout_target="main"
        )
