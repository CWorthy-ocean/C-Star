...
def test_write_to_config_file(tmp_path):
    """Tests `_write_to_config_file` for correct behavior when creating and appending to
    a configuration file.

    This test verifies:
    - If `_CSTAR_CONFIG_FILE` does not exist, `_write_to_config_file` creates the file
      with base configuration content prepended to `config_file_str`.
    - If `_CSTAR_CONFIG_FILE` exists, `_write_to_config_file` appends `config_file_str`
      without duplicating the base content.
    ...
    Mocks
    -----
    `_CSTAR_CONFIG_FILE` : pathlib.Path
        Mocked to point to a temporary file within `tmp_path`, ensuring no actual filesystem
        files are modified.

    Asserts
    -------
    - The file content after the first write operation matches the expected base content
      plus `initial_config_content`.
    - The file content after the second write operation matches the previous content with
      `additional_config_content` appended, verifying no duplication of the base content.

    Steps
    -----
    1. Patch `_CSTAR_CONFIG_FILE` to point to a temporary file within `tmp_path`.
    2. Call `_write_to_config_file` with `initial_config_content`, then verify the file
       contains the base configuration content followed by `initial_config_content`.
    3. Call `_write_to_config_file` again with `additional_config_content` and check that
       it correctly appends the new content without altering the existing content.
    """
    # Define the test config content and path for the temporary file
    initial_config_content = "   print('hello world!')"
    additional_config_content = "   print('... you still here?')"
    test_config_file = tmp_path / "_cstar_config_file.py"

    # Patch `_CSTAR_CONFIG_FILE` to point to the temporary test file
    with mock.patch("cstar.base.utils._CSTAR_CONFIG_FILE", new=test_config_file):
        # Call the function
        _write_to_config_file(initial_config_content)

        # Read the content of the file
        with open(test_config_file, "r") as f:
            written_content = f.read()

        # Define the expected content that should be in the file
        expected_base_conf_str = (
        ...
        expected_initial_content = expected_base_conf_str + initial_config_content
        assert written_content == expected_initial_content

        # Second call to the function now file exists
        _write_to_config_file(additional_config_content)

        # Read the content of the file after the second write
        with open(test_config_file, "r") as f:
            final_written_content = f.read()

        # Define the expected content after the second write
        expected_final_content = expected_initial_content + additional_config_content

        # Assert the content matches expected after the second write
        assert final_written_content == expected_final_content
