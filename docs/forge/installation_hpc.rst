.. _forge-installation-hpc:

Advanced installation (HPC or developers)
==========================================

.. note::

   This page was carried over from the standalone ``cstar-forge`` documentation
   (0.8.2) when Forge moved into C-Star, and has not yet been reconciled with
   :doc:`../installation` and :doc:`../machines`. It still refers to the ``cstar-forge`` package, its developer
   setup and its old system tags; where it disagrees with those pages,
   they are authoritative. Forge now ships inside ``cstar-ocean``.

For a laptop or workstation, the one-command conda install in
:doc:`getting_started` is all you need. This page covers the
...
:ref:`lockfile replay <reproducible-installs-lockfile>` below instead of a fresh
solve.
...
``forge-blueprint-wizard.ipynb`` notebook instead of the Voila web app.
...
Fork `CWorthy-ocean/cstar-forge <https://github.com/CWorthy-ocean/cstar-forge>`__,
then:
...
.. note::
   ...
   `GitHub SSH documentation <https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent>`__.
...
`Pixi <https://pixi.sh>`__ reads dependencies straight from ``pyproject.toml``
...
.. _reproducible-installs-lockfile:
...
The `conda-lockfiles <https://github.com/conda/conda-lockfiles>`__ plugin aims to
