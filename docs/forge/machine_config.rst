...
.. note::
   ...
   :doc:`../machines`, :doc:`../configuration` and the ``CSTAR_CATALOG`` notes in :doc:`catalog`. It still refers to the ``cstar-forge`` package, its developer
...
:doc:`internals` §2–4 for the full authoring/execution split. This is Forge's
...
- :doc:`internals` - module map, including ``config.py`` (``DataPaths``/``resolve_host()``)
