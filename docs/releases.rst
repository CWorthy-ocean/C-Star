...
This release provides basic functionality, including the ability to create, import, and export "blueprints" for reproducible ocean model simulations (C-Star Cases) using supported ocean models (ROMS, optionally with MARBL biogeochemistry) and run those Cases locally or on supported HPC systems (via Slurm and PBS). 
