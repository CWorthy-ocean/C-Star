...
| `identity.grid_name` | **multiple**: DomainSpec's `Domain.yml` has its own `grid_name` field (e.g. `"ccs-12km"`), which the wizard/resolver passes through as `identity.grid_name` | **two places**: `self.grid_name` Text (1274, manual) AND auto-overwritten by `_on_domain` (1820) when a catalog Domain is picked | feeds `name`/`casename`/output filenames | — |
| `identity.ensemble_id` | per-run (not cataloged) | `self.ensemble` Text, optional int (1432) | appended to `name` (`_{:03d}`) | — |
| `identity.description` | **multiple**: `ForgeBlueprint` default is `"Generated blueprint"`; DomainSpec's `Domain.yml` also carries a `description` | `self.description` Text (1425) — a plain manual field, not auto-filled from the DomainSpec's own description on domain pick | descriptive only; not consumed by namelist/cppdefs | — |
| `run.start_date` / `run.end_date` | DomainSpec (`Domain.yml` `start_time`/`end_time` prefill) — editable per-run | **two places**: `self.start`/`self.end` DatePickers (1407, 1413, manual) AND auto-set from a catalog Domain's `start_time`/`end_time` in `_on_domain` (1830) | drives `time_stepping.ntimes` (R, via CFL/duration calc); roms-tools `start_time`/`end_time`/`ini_time` kwargs (all forcing generators) | (R) `ntimes` is pure-derived from `(end-start).days` and `dt` |
...
| `working_dir` | derived — host/location only | not exposed in the wizard at all; not even a `build_forge_blueprint` parameter | executor artifact root; excluded from `content_hash` | (R) sentinel `~/cstar-forge-data` expands to `<root>/<name>` on validation |
| `datasets` (top-level list) | derived from `forcing.resolved_datasets` | not directly editable (recomputed from forcing selections) | `forge_blueprint_to_builder_kwargs` → `source_dataset_keys=` → `ensure_source_data()` | (R) resolver recomputes fully from the resolved forcing/topography sources — never hand-authored |
...
| Field | Piece(s) | Wizard UI | Executor destination | Overridden? |
|---|---|---|---|---|
| `cppdefs.obc_{west,east,north,south}` | derived from `domain.open_boundaries` (DomainSpec) | technically reachable in the generic Advanced-settings `cppdefs` pane (see below), but pointless to hand-edit — always overwritten by (R)/(G) | `cppdefs.opt.j2` → `#define OBC_{WEST,EAST,NORTH,SOUTH}` | (R) + (G), see §2 open_boundaries row — always overwritten, never a real ModelSpec default despite `model.yml` carrying placeholder `true` values |
| `cppdefs.marbl` | ModelSpec (`model.yml`, `true`) | Advanced-settings `cppdefs` pane (generic editor, see below) | **NOT read by `cppdefs.opt.j2`** — `MARBL`/`MARBL_DIAGS` are unconditional `#define`s in the template regardless of this value. It IS read at `input_data.py:875` to gate whether the run-time `bgc` section gets touched at all (`has_bgc_compile`). `forge_blueprint_resolve.marbl_from_model_settings()` also reads it but **is dead code — not called anywhere** in `cstar_forge/` or `tests/` | not overridden; effectively a "is this a BGC model" flag consumed only by input_data.py, not compile-time codegen |
...
These are exactly the sections in `forge_blueprint_resolve.OUTPUT_SECTIONS` +
`OUTPUT_MARBL_FIELDS` — deep-merged over the ModelSpec defaults at Phase 1
(`_deep_merge(settings, output_settings)`, forge_blueprint_resolve.py ~L358), **before**
`run_time_overrides`/`compile_time_overrides` are applied (so a wizard hand-edit still
wins over the OutputSpec selection).
...
- `forge_blueprint_resolve.marbl_from_model_settings()` is defined but has zero call
  sites in `cstar_forge/` or `tests/` — either wire it in (e.g. as the single source
  for `has_bgc_compile` in `input_data.py`, which currently duplicates the same
  one-liner inline) or delete it.
- `docs/reference-settings-run-time.md` / `reference-settings-compile-time.md` MyST
  pages `{include}` files (`templates/run-time-defaults.yml`,
  `templates/compile-time-defaults.yml`) that no longer exist post-consolidation —
