...
`roms-marbl-0.6-default` pins `0.6.4`, `roms-marbl-0.7-default` pins `0.7.0`,
`roms-marbl-0.8-default` pins `0.8.0` (adds the `parabolic_splines`/
`upstream_ts_land_curv` advection cppdefs flags, PR #361, with no new settings
tier — it still resolves to `RunTimeSettingsV0_7_0`); older specs stay
