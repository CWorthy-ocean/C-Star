...
- **GLORYS**: Global or regional ocean initial conditions; subset and time-extract logic depends on whether the request is regional (grid-based, dataset key `GLORYS_REGIONAL`) or global (`GLORYS_GLOBAL`). Returns a single Path (for a single day) or `List[Path]` (for multiple days).
...
- **CONSTANTS**, **DAI**: Streamed/auto-downloaded by roms-tools itself at generation time — Forge never stages a local path for these.
