...
> ForgeBlueprint/wizard refactor — the "planned refactor" it describes is now largely
> implemented (`CstarSpecBuilder` → `ForgeBlueprint` + `ForgeExecutor`; `cstar_forge/forge/`
> is the relocatable processing engine). File paths and class names below (e.g.
> `CstarSpecBuilder`, `cstar_forge/forge_blueprint.py`, `sources.*`) reflect the design as
> originally proposed and have since moved or been renamed (`ForgeBlueprint` now lives at
> `cstar_forge/forge/forge_blueprint.py`; `sources` was renamed `forcing`). Kept for the
...
`cstar_forge/forge_blueprint.py` (`ForgeBlueprint`).
...
**Governing principle:** the authoritative `ForgeBlueprint` stores ONLY
...
**Stored in `ForgeBlueprint`** (curated inputs + reviewable settings):
...
| **Naming** | `name`, `casename`, namelist `title.casename` | `f(model_name, grid_name, ensemble_id, dates, n_procs)` — `ForgeBlueprint` properties |
| **Artifacts** | `s_coord.{theta_s,theta_b,tcline}`; `grid`/`initial`/`forcing` file paths | generated grid file / generated NetCDF |

> **Design rule:** `ForgeBlueprint` is **input-only**. The existing blueprint YAML remains
...
`run_output_dir` are deterministic functions exposed as `ForgeBlueprint` properties /
...
| Piece | Catalog home | Contributes to `ForgeBlueprint` |
|---|---|---|
...
`(ForgeBlueprint, host) → artifacts`, where `host` (machine config + data paths) is
...
2. **[DONE — draft]** Phase-2 processing engine
   (`cstar_forge/forge_blueprint_engine.py`, `tests/test_forge_blueprint.py`): ingest a
   `ForgeBlueprint`, resolve the **host** (machine + data paths) from `cstar_forge.config`
...
2b. **[DONE — schema identity]** ``ForgeBlueprint`` carries an ``application`` discriminator
   (default ``"roms_marbl"`` — the target C-Star app) and ``forge_blueprint_version``;
   ``from_yaml`` rejects files declaring a newer version. A portability guard test keeps
   ``forge_blueprint.py`` free of ``cstar_forge``/``cstar`` imports (so it stays
   ...
   **[TODO — at the C-Star migration, NOT before]** Add a *byte-exact golden namelist*
   test: run ``process_forge_blueprint`` on the example and assert the generated
   ...
   ``tests/test_forge_blueprint.py``.) Until then, behavior is guarded by the resolver↔builder
...
3. Reconcile the Phase-1 resolver with the live builder so the two paths can't drift.
   - **[DONE — parity net]** `TestResolverBuilderParity` (`tests/test_forge_blueprint.py`,
     `@pytest.mark.integration`) builds a `ForgeBlueprint` two ways for several domains —
   ...
   - **[TODO — consolidate]** Have `CstarSpecBuilder.model_post_init` *delegate* to
     `build_forge_blueprint` (one source of truth) rather than re-derive in parallel.
...
4. Keep `CstarSpecBuilder` as a thin facade for back-compat: `builder.forge_blueprint`
   returns the resolved `ForgeBlueprint`; `builder.generate_inputs()` delegates to
   `process_forge_blueprint`.
5. Add `ForcingSpec` to the catalog and wire the composable-piece selection (model /
   domain / forcing) + the UI on top of `build_forge_blueprint`.
...
* **Snapshot into `ForgeBlueprint`** — the data-source identifiers (GLORYS `dataset_id`,
