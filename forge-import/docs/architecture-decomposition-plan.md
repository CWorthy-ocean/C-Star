# Decomposition plan: `CstarSpecBuilder` → the forge application (SpecConfig-native executor)
...
- **forge application** — *what we are building.* Blueprint = the **`SpecConfig`**. When it
...
So: **forge blueprint = SpecConfig** (our input); **roms_marbl blueprint = an emitted
...
Schema iteration is orthogonal: it changes field *definitions inside* `SpecConfig`, not
the schema/executor/resolver/UI boundaries D establishes.

The seams already exist and were built for this: `SpecConfigExecutor` Protocol +
injectable `executor_factory` + `process_spec_config` (`spec_config_engine.py`),
`spec_config.py` portability guard, `application` discriminator.
...
cstar_forge/                         # AUTHORING (stays in Forge)
  ...
  spec_config_wizard.py                        # wizard UI
  ...
  forge_app/                         # THE FORGE APPLICATION (relocatable to C-Star as one unit)
    spec_config.py                   # the forge blueprint (already portable) + unified item models
    executor.py                      # SpecConfig-native executor (ex-CstarSpecBuilder guts)
    engine.py                        # process_spec_config + SpecConfigExecutor Protocol
...
(`catalog*`, `spec_config_resolve`, `spec_config_wizard`, `config`). Forge authoring
...
|---|---|
| Constructor fields / schema; `from_domain` | **Delete** — `SpecConfig` is the input; `from_spec_config` is the ctor |
...
| Persistence (`persist`, `_persist_settings`, `_load_settings*`, `_convert_paths*`, `dump`, `load`) | `SpecConfig.to_yaml/from_yaml` is the canonical **forge** blueprint; **retire `dump`/`load`** |
...
| `name`, `n_procs`, `casename`, `datestr` | Already on `SpecConfig`; executor reads them |
...
`build_spec_config` → `SpecConfig` → `process_spec_config`** (drops the `from_domain`
...
- Extend the existing `spec_config.py` portability guard to the whole future package.
- **Value:** free safety net + documents the target boundary for the invitees.
- **Risk:** none. **Test impact:** +1 guard test.

### Phase B — builder becomes SpecConfig-native (the commitment point)
1. Add `CstarSpecBuilder.from_spec_config(cfg)` as canonical ctor; construct directly
   from `SpecConfig` (drop the lossy `spec_config_to_builder_kwargs` bridge).
...
4. **Retire the parity test** (`test_spec_config.py:771`) — one derivation now.
...
- Create `cstar_forge/forge_app/`; move `input_data.py`, `source_data.py`,
  `source_registry.py`, `settings.py`, `spec_config.py`, `spec_config_engine.py`
...
- Fold `models.py` forcing item models into `spec_config.py` (single source). Reconcile:
  - **`extra` policy:** `models.py` items use `extra="allow"` (legacy passthrough);
    `spec_config` uses `forbid`. Move to `forbid` now that `options` is the sanctioned
  ...
  - **`SourceSpec` validator:** move the `Literal` typing + `glorys_layout`-only-for-GLORYS
    validator into `spec_config`'s `SourceSpec` (stays stdlib/pydantic-only).
...
1. **Behavior now covered by `test_spec_config.py`** (resolver/schema/derivation) →
   delete or thin to a smoke check.
2. **Genuinely executor-specific** (path resolution, blueprint writing, settings merge,
   input-generation orchestration) → re-point to an executor built via `from_spec_config`,
...
Introduce one test helper — `make_executor(**overrides)` = `build_spec_config(...)` →
`from_spec_config(...)` — so future signature changes touch one site, not 69.
...
1. **`application` discriminator** — `DEFAULT_APPLICATION = "forge"` (done in
   `spec_config.py`). C-Star routes the SpecConfig blueprint to the `forge` application.
...
> Follow-up: `examples/spec_config*.yml` and `docs/spec-config-example.test-tiny.yml` were
...
- No change to generated outputs (netCDFs, namelist, **downstream roms_marbl blueprint**) —
  the deferred byte-golden namelist test (`test_spec_config.py:146`) is the eventual proof;
