...
class TestForcingInput:
    ...
    def test_forcinginput_creation_complete(self):
        ...
        tidal_item = TidalForcingItem(source=SourceSpec(name="TPXO"), ntides=15)
        river_item = RiverForcingItem(
            source=SourceSpec(name="DAI", climatology=False), include_bgc=True
        )
    ...
    def test_forcinginput_creation_multiple_items(self):
        ...
        surface_items = [
            SurfaceForcingItem(
                source=SourceSpec(name="ERA5"), type="physics", correct_radiation=True
            ),
            SurfaceForcingItem(
                source=SourceSpec(name="UNIFIED", climatology=True), type="bgc"
            ),
        ]
        boundary_items = [
            BoundaryForcingItem(source=SourceSpec(name="GLORYS"), type="physics"),
            BoundaryForcingItem(
                source=SourceSpec(name="UNIFIED", climatology=True), type="bgc"
            ),
        ]
        forcing = ForcingInput(surface=surface_items, boundary=boundary_items)
...
class TestModelInputs:
    ...
    def test_modelinputs_creation_complete(self):
        ...
        tidal_items = [TidalForcingItem(source=SourceSpec(name="TPXO"), ntides=15)]
        river_items = [
            RiverForcingItem(
                source=SourceSpec(name="DAI", climatology=False), include_bgc=True
            )
        ...
        data = {
            ...
            "forcing": {
                "surface": [
                    ...
                    {"source": {"name": "UNIFIED", "climatology": True}, "type": "bgc"},
                ],
                "boundary": [
