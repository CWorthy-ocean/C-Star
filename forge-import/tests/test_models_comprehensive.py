...
"""
        ...
        assert (
            stage.settings_dict["section1"]["subsection"]["nested_param"]
            == "nested_value"
        )
...
class TestModelSpec:
    ...
    def test_modelspec_creation_minimal(self):
        ...
        surface_item = SurfaceForcingItem(
            source=SourceSpec(name="ERA5"), type="physics"
        )
        boundary_item = BoundaryForcingItem(
            source=SourceSpec(name="GLORYS"), type="physics"
        )
        forcing = ForcingInput(surface=[surface_item], boundary=[boundary_item])
        inputs = ModelInputs(grid=grid, initial_conditions=ic, forcing=forcing)
    ...
    def test_modelspec_cross_validation_missing_settings_key(self, tmp_path):
        ...
        assert (
            "cppdefs" in str(exc_info.value)
            or "sections" in str(exc_info.value).lower()
        )
    ...
    def test_modelspec_template_file_validation_existing_file(self, tmp_path):
        ...
        templates = TemplatesSpec(
            compile_time=cstar_models.CodeRepository(
                location=str(template_dir),
                branch="main",
                filter=cstar_models.PathFilter(
                    files=["cppdefs.opt.j2", "param.opt.j2", "Makefile"]
                ),
            )
        )
...
class TestExtractSourceName:
    ...
    def test_extract_source_name_from_dict(self):
        ...
        assert (
            _extract_source_name({"name": "UNIFIED", "climatology": True}) == "UNIFIED"
        )
...
class TestDatasetKeysFromInputs:
    ...
    def test_dataset_keys_from_inputs_basic(self, mock_registry, mock_map):
        ...
        mock_registry.__contains__ = lambda self, key: (
            key in {"GLORYS_REGIONAL", "UNIFIED_BGC", "ERA5", "SRTM15_V2.7"}
        )
    ...
    def test_dataset_keys_from_inputs_with_tidal_river(self, mock_registry, mock_map):
        ...
        mock_map.side_effect = lambda x: {"TPXO": "TPXO", "DAI": "DAI"}.get(
            x, x.upper()
        )
        ...
        tidal_item = TidalForcingItem(source=SourceSpec(name="TPXO"))
        river_item = RiverForcingItem(source=SourceSpec(name="DAI"))
    ...
    def test_dataset_keys_from_inputs_no_source_data(self):
        ...
        with patch(
            "cstar_forge.forge.source_data.map_source_to_dataset_key",
            side_effect=AttributeError("Function not available"),
        ):
            with pytest.raises(ValueError) as exc_info:
                _dataset_keys_from_inputs(inputs)
            assert "source_data module functions are not available" in str(
                exc_info.value
            )
...
class TestCollectDatasets:
    ...
    def test_collect_datasets_explicit_only(self, mock_extract):
        ...
        block = {"datasets": ["GLORYS_REGIONAL", "UNIFIED_BGC"]}
            ...
            "test_model": {
                ...
                "inputs": {
                    ...
                    "forcing": {
                        ...
                        "boundary": [{"source": {"name": "GLORYS"}, "type": "physics"}],
                    },
                },
            }
        }
            ...
            "test_model": {
                ...
                "templates": {
                    "compile_time": {
                        ...
                        "filter": {"files": ["cppdefs.opt.j2", "param.opt.j2"]},
                    },
                    "run_time": {
                        "location": "templates/run-time",
                        "filter": {"files": ["namelist.nml.j2"]},
                    },
                },
                "inputs": {
                    "grid": {"topography_source": "ETOPO5"},
